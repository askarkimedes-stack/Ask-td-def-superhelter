...
<!-- Version: #7 -->
<!-- Filnavn: script.js -->
<!-- Handling: Erstatt HELE script.js med denne koden -->
...
let config = {
    ...
    bank: { dmg: 0, range: 120, speed: 180, cost: 200, color: '#f1c40f' }, // Speed = antall frames mellom utbetaling (ca 3 sek)
    enemy: { hpBase: 5, hpScale: 3, speed: 1.0 }, 
    waves: { startCount: 4, growth: 1 }
};

// Evolusjons-data (Evos)
const evoData = {
    boom: [
...
let summons = [];
let particles = [];
let floatingTexts = []; // For å vise gull-inntjening
...
class Hero {
    constructor(x, y, type) {
        ...
        // Cooldown for Bank = genereringstid
...
function updateGameLogic() {
    ...
    enemies.forEach((e, i) => { 
        ...
        if(e.hp <= 0) { 
            ...
            // Tax Collector Evo Logic (Bank)
            heroes.forEach(h => {
                if(h.type === 'bank' && h.isEvolved === 'tax' && Math.hypot(h.x - e.x, h.y - e.y) < h.range) {
                    gold += 5; // Stjel 5g ekstra
                    showFloatText(h.x, h.y - 30, "+5g", "#00ff00");
                }
            });
...
function gameLoop() {
    ...
    // Kjør logikk flere ganger basert på speed
...
function applyUpgrade() {
    ...
    updateUI(); 
    selectHero(selectedHero); // Oppdater pris i UI
}
